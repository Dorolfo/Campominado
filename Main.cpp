/*
ALUNO:
Rodolfo Constâncio de Almeida Lage    2016017890
*/
#include <iostream>
#include <windows.h>
#include <cstdlib>
...
int main()
{
    ...
    ALLEGRO_BITMAP *Img_Menu = al_load_bitmap("Jogar.jpg"); //DEFINE MENU COMO PLANO DE FUNDO
    ...
    ALLEGRO_BITMAP * QuadradoFechado = al_load_bitmap("Fechado.jpg");
    if (!QuadradoFechado)
    {
    ...
    ALLEGRO_BITMAP * QuadradoAberto = al_load_bitmap("Aberto.jpg");
    if (!QuadradoAberto)
    {
    ...
    ALLEGRO_BITMAP * Bandeira_Img = al_load_bitmap("Bandeira.jpg");
    if (!Bandeira_Img)
    {
    ...
    ALLEGRO_BITMAP *img_bomba = al_load_bitmap("Bomba.jpg");
    if (!img_bomba)
    {
        ...
        if(Game_Over == 0) // JOGO CONTINUA
        {
            ...
            if (evento.type == ALLEGRO_EVENT_MOUSE_BUTTON_UP) //SE TIVE UM CLIQUE
            {
                ...
                if(evento.mouse.button & 1)//CLIQUE COM O BOTAO ESQUERDO
                {
                    if(!CM.VerificaAberto(i,j)) //VERIFICA SE A CASA EST� FECHADA
                    {
                        if(CM.VerificaBomba(i,j)) //VERIFICA SE A CASA EM QUESTAO TEM BOMBA
                        {
                            ...
                            Img_final = al_load_bitmap("Perdeu.png");
                ...
                if (Quant_Bandeiras == Quant_Bombas && CM.QuantAberto() == (Linhas*Colunas)- Quant_Bombas)
                {
                    ...
                    Img_final = al_load_bitmap("Venceu.png");
